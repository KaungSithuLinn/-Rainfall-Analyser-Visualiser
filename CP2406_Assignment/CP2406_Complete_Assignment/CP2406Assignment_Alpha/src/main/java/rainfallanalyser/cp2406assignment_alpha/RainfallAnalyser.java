 ...
 *
 * Link for the project repo: https://github.com/KaungSithuLinn/Rainfall-Analyser-Visualiser.git
 */
public class RainfallAnalyser {
     ...
     */
    private static void saveRainfallData(ArrayList<String> analysedRainfallData, String filename) throws IOException {
        System.out.println("Choose the file format to save the analysed data:");
        System.out.println("1. CSV");
        System.out.println("2. Text (TXT)");
        int choice = TextIO.getInt();

        if (choice == 1) {
            saveAsCSV(analysedRainfallData, filename);
        } else if (choice == 2) {
            saveAsTXT(analysedRainfallData, filename);
        } else {
            System.out.println("Invalid choice. Data will not be saved.");
        }
    }
    /**
     * Save the analysed rainfall data as a CSV file
     */
    private static void saveAsCSV(ArrayList<String> analysedRainfallData, String filename) throws IOException {
        String savePath = getSavePath(filename, "csv");

        try (CSVPrinter csvPrinter = new CSVPrinter(new FileWriter(savePath), CSVFormat.DEFAULT)) {
            csvPrinter.printRecord("year", "month", "total", "minimum", "maximum");
            for (String rainfallData : analysedRainfallData) {
                csvPrinter.printRecord(rainfallData);
            }
        }

        System.out.println("Analysed rainfall data saved as CSV: " + savePath);
    }

    /**
     * Save the analysed rainfall data as a TXT file
     */
    private static void saveAsTXT(ArrayList<String> analysedRainfallData, String filename) throws IOException {
        String savePath = getSavePath(filename, "txt");

        try (FileWriter fileWriter = new FileWriter(savePath)) {
            fileWriter.write("year,month,total,minimum,maximum\n");
            for (String rainfallData : analysedRainfallData) {
                fileWriter.write(rainfallData + "\n");
            }
        }

        System.out.println("Analysed rainfall data saved as TXT: " + savePath);
    }
    ...
    /**
     * Identify the data sets on rainfall that can be analyzed.
     * Lets the user choose from this list which data set to examine.
     */
    ...
    /**
     * Return the location of a file containing analyzed rainfall data.
     */
    private static String getSavePath(String filename, String extension) {
        String[] filenameElements = filename.trim().split("\\.");
        String saveDir = "src/main/resources/rainfalldata_analysed/";
        return saveDir + filenameElements[0] + "_analysed." + extension;
    }
}
